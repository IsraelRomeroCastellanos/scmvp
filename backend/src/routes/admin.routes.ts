...
const adminRoutes = (pool: Pool) => {
  ...
  // ✅ Obtener un usuario
  router.get('/api/admin/usuarios/:id', async (req: Request, res: Response) => {
    const { id } = req.params;
    try {
      const result = await pool.query(
        'SELECT id, email, nombre_completo, rol, empresa_id, activo FROM usuarios WHERE id = $1',
        [id]
      );
  ...
  router.post('/api/admin/usuarios/:id/reset-password', async (req: Request, res: Response) => {
    const { id } = req.params;
