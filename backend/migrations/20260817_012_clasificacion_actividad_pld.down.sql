...
BEGIN
  IF pg_catalog.current_schema() IS DISTINCT FROM 'public'
    ...
    RAISE EXCEPTION 'DOWN bloqueado: faltan esquema, tablas o migration key 012';
  END IF;

  IF EXISTS (SELECT 1 FROM public.clasificacion_actividad_pld_item)
     OR EXISTS (SELECT 1 FROM public.clasificacion_actividad_pld_version) THEN
    RAISE EXCEPTION 'DOWN bloqueado: existen clasificaciones de actividad que se perderian';
  END IF;
  ...
  LOOP
    IF NOT EXISTS (
      ...
      RAISE EXCEPTION 'DOWN bloqueado: %.% incompatible',expected.tabla,expected.columna;
    END IF;
  END LOOP;
  ...
  IF (SELECT pg_catalog.count(*) FROM pg_catalog.pg_attribute a
    ...
    RAISE EXCEPTION 'DOWN bloqueado: cantidad de columnas incompatible';
  END IF;
  ...
  IF expression_sql IS NULL THEN RAISE EXCEPTION 'DOWN bloqueado: falta default activa'; END IF;
  ...
  IF evaluated_boolean IS DISTINCT FROM false THEN
    RAISE EXCEPTION 'DOWN bloqueado: default activa incompatible';
  END IF;
  ...
  LOOP
    ...
    IF expression_sql IS NULL THEN RAISE EXCEPTION 'DOWN bloqueado: falta default %.%',expected.tabla,expected.columna; END IF;
    EXECUTE pg_catalog.format('SELECT (%s)::timestamptz',expression_sql) INTO evaluated_timestamp;
    IF evaluated_timestamp IS NULL THEN RAISE EXCEPTION 'DOWN bloqueado: default temporal incompatible'; END IF;
  END LOOP;
  ...
  LOOP
    ...
    IF actual_columns IS DISTINCT FROM expected.columnas
      ...
      RAISE EXCEPTION 'DOWN bloqueado: constraint % incompatible',expected.nombre;
    END IF;
  END LOOP;
  ...
  LOOP
    SELECT pg_catalog.pg_get_expr(c.conbin,c.conrelid) INTO expression_sql
     ...
     WHERE c.conrelid=pg_catalog.to_regclass('public.' || expected.tabla)
       ...
       AND c.convalidated AND NOT c.condeferrable;
    IF expression_sql IS NULL THEN RAISE EXCEPTION 'DOWN bloqueado: CHECK % ausente',expected.nombre; END IF;
    ...
    IF accepted IS DISTINCT FROM expected.esperado THEN
      RAISE EXCEPTION 'DOWN bloqueado: semantica de % incompatible',expected.nombre;
    END IF;
  END LOOP;
  ...
  LOOP
    ...
    IF actual_columns IS DISTINCT FROM expected.columnas THEN
      RAISE EXCEPTION 'DOWN bloqueado: indice % incompatible',expected.nombre;
    END IF;
  END LOOP;
  ...
  IF expression_sql IS NULL THEN RAISE EXCEPTION 'DOWN bloqueado: falta predicado activo'; END IF;
  ...
  IF accepted IS DISTINCT FROM ARRAY[true,false]::BOOLEAN[] THEN
    RAISE EXCEPTION 'DOWN bloqueado: predicado activo incompatible';
  END IF;
  ...
  IF (SELECT pg_catalog.count(*) FROM pg_catalog.pg_constraint c
    ...
    RAISE EXCEPTION 'DOWN bloqueado: cantidad de objetos inesperada';
  END IF;
END
$$;

DROP TABLE public.clasificacion_actividad_pld_item;
DROP TABLE public.clasificacion_actividad_pld_version;
