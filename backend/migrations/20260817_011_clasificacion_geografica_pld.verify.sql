...
BEGIN
  ...
  IF pg_catalog.to_regclass('public.clasificacion_geografica_pld_version') IS NULL
    ...
    RAISE EXCEPTION 'VERIFY fallido: faltan tablas o migration key 011';
  END IF;
  ...
  IF expression_sql IS NULL THEN
    RAISE EXCEPTION 'VERIFY fallido: falta default activa';
  END IF;
  ...
  SELECT pg_catalog.pg_get_expr(i.indpred,i.indrelid) INTO expression_sql
    ...
    JOIN pg_catalog.pg_class x ON x.oid=i.indexrelid
    JOIN pg_catalog.pg_namespace n ON n.oid=x.relnamespace
   WHERE n.nspname='public' AND x.relname='uq_clasificacion_geografica_pld_version_tipo_activa';
  IF expression_sql IS NULL THEN
    RAISE EXCEPTION 'VERIFY fallido: falta predicado de unicidad activa';
  END IF;
  EXECUTE pg_catalog.format(
    'SELECT pg_catalog.array_agg((%s) IS NOT FALSE ORDER BY n) FROM (VALUES (1,true),(2,false)) v(n,activa)',
    expression_sql
  ) INTO accepted;
  IF accepted IS DISTINCT FROM ARRAY[true,false]::BOOLEAN[] THEN
    RAISE EXCEPTION 'VERIFY fallido: predicado de unicidad activa incompatible';
  END IF;
  ...
  LOOP
    SELECT pg_catalog.pg_get_expr(i.indpred,i.indrelid) INTO expression_sql
     ...
     WHERE n.nspname='public' AND x.relname=expected.nombre;
    IF expression_sql IS NULL THEN
      RAISE EXCEPTION 'VERIFY fallido: falta predicado de indice %',expected.nombre;
    END IF;
    EXECUTE pg_catalog.format(expected.consulta,expression_sql) INTO accepted;
    IF accepted IS DISTINCT FROM expected.esperado THEN
      RAISE EXCEPTION 'VERIFY fallido: predicado de indice % incompatible',expected.nombre;
    END IF;
  END LOOP;
