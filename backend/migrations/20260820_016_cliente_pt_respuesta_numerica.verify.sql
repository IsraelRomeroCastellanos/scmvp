...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'VERIFY fallido: falta migration key 016';
  END IF;
  IF (
    SELECT pg_catalog.count(*)
    FROM (VALUES
      ('matriz_opcion_id','pg_catalog.int4'::pg_catalog.regtype,false),
      ('matriz_rango_id','pg_catalog.int4'::pg_catalog.regtype,false),
      ('valor_numerico','pg_catalog.numeric'::pg_catalog.regtype,false),
      ('unidad','pg_catalog.varchar'::pg_catalog.regtype,false)
    ...
    RAISE EXCEPTION 'VERIFY fallido: columnas de respuesta numerica incompatibles';
  END IF;
  ...
  IF NOT EXISTS (
    ...
    WHERE c.conrelid='public.cliente_pt_respuesta'::pg_catalog.regclass
      ...
      AND c.confrelid='public.matriz_rango'::pg_catalog.regclass
    ...
    RAISE EXCEPTION 'VERIFY fallido: UNIQUE o FK compuesta de rango incompatible';
  END IF;
  ...
  EXECUTE pg_catalog.format(
    ...
    '(6,NULL::int,1::int,NULL::numeric,''UMA''::varchar),'
    '(7,NULL::int,1::int,10.5::numeric,''MONTO''::varchar),'
    '(8,NULL::int,1::int,''NaN''::numeric,''UMA''::varchar),'
    '(9,NULL::int,1::int,''Infinity''::numeric,''UMA''::varchar),'
    '(10,NULL::int,1::int,''-Infinity''::numeric,''UMA''::varchar)) '
    'AS cliente_pt_respuesta(prueba,matriz_opcion_id,matriz_rango_id,valor_numerico,unidad)',
    tipo_expr
  ) INTO tipo_tests;
  IF tipo_tests IS DISTINCT FROM
     ARRAY[TRUE,TRUE,TRUE,FALSE,FALSE,FALSE,FALSE,FALSE,FALSE,FALSE]::BOOLEAN[] THEN
    RAISE EXCEPTION 'VERIFY fallido: semantica de exclusividad opcion/rango incompatible';
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION 'VERIFY fallido: existe una respuesta que no cumple el contrato';
  END IF;
  IF NOT EXISTS (
