...
BEGIN
  ...
  FOR esperado IN SELECT * FROM (VALUES
  ...
  ) AS x(tabla,nombre,columnas,unico,tipo_predicado) LOOP
    ...
    IF NOT FOUND OR real.unico IS DISTINCT FROM esperado.unico
       OR real.parcial IS DISTINCT FROM (esperado.tipo_predicado IS NOT NULL)
       ...
       OR real.tiene_expresiones OR real.columnas IS DISTINCT FROM esperado.columnas THEN
    ...
    IF esperado.tipo_predicado = 'activa' THEN
      ...
      END;
    ELSIF esperado.tipo_predicado = 'pendiente' THEN
      BEGIN
        EXECUTE pg_catalog.format(
          'SELECT pg_catalog.array_agg((%s) IS TRUE ORDER BY orden) '
          'FROM (VALUES '
    ...
    IF NOT predicado_valido THEN
      RAISE EXCEPTION 'VERIFY fallido: predicado semantico del indice public.% incompatible', esperado.nombre;
    END IF;
  END LOOP;
