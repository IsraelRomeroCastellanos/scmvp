...
BEGIN
  ...
  IF pg_catalog.to_regclass('public.cliente_gr_evaluacion') IS NULL
    ...
    RAISE EXCEPTION 'VERIFY fallido: faltan tablas o migration key 010';
  END IF;
  ...
  IF (SELECT pg_catalog.count(*) FROM pg_catalog.pg_attribute a
     ...
     OR (SELECT pg_catalog.count(*) FROM pg_catalog.pg_attribute a
       WHERE a.attrelid='public.cliente_gr_criterio_resultado'::pg_catalog.regclass
         AND a.attnum>0 AND NOT a.attisdropped) <> 10 THEN
  ...
  LOOP
    ...
    IF evaluated_timestamp IS NULL THEN
      RAISE EXCEPTION 'VERIFY fallido: default temporal incompatible';
    END IF;
  END LOOP;

  FOR expected IN
    SELECT * FROM (VALUES
      ('cliente_gr_evaluacion','pk_cliente_gr_evaluacion','p',ARRAY['id']::TEXT[],NULL::TEXT,NULL::TEXT[]),
      ('cliente_gr_evaluacion','uq_cliente_gr_evaluacion_cliente_version','u',ARRAY['cliente_id','numero_version']::TEXT[],NULL::TEXT,NULL::TEXT[]),
      ('cliente_gr_evaluacion','uq_cliente_gr_evaluacion_id_matriz','u',ARRAY['id','matriz_version_id']::TEXT[],NULL::TEXT,NULL::TEXT[]),
      ('cliente_gr_evaluacion','fk_cliente_gr_evaluacion_cliente_empresa','f',ARRAY['cliente_id','empresa_id']::TEXT[],'clientes',ARRAY['id','empresa_id']::TEXT[]),
      ('cliente_gr_evaluacion','fk_cliente_gr_evaluacion_matriz_empresa','f',ARRAY['matriz_version_id','empresa_id']::TEXT[],'matriz_empresa_version',ARRAY['id','empresa_id']::TEXT[]),
      ('cliente_gr_evaluacion','fk_cliente_gr_evaluacion_pt_cliente_empresa_matriz','f',ARRAY['pt_evaluacion_id','cliente_id','empresa_id','matriz_version_id']::TEXT[],'cliente_pt_evaluacion',ARRAY['id','cliente_id','empresa_id','matriz_version_id']::TEXT[]),
      ('cliente_gr_evaluacion','fk_cliente_gr_evaluacion_resultado_matriz_ambito','f',ARRAY['matriz_resultado_id','matriz_version_id','ambito']::TEXT[],'matriz_resultado',ARRAY['id','matriz_version_id','ambito']::TEXT[]),
      ('cliente_gr_evaluacion','fk_cliente_gr_evaluacion_creada_por','f',ARRAY['creada_por']::TEXT[],'usuarios',ARRAY['id']::TEXT[]),
      ('cliente_gr_criterio_resultado','pk_cliente_gr_criterio_resultado','p',ARRAY['id']::TEXT[],NULL::TEXT,NULL::TEXT[]),
      ('cliente_gr_criterio_resultado','uq_cliente_gr_criterio_resultado_evaluacion_criterio','u',ARRAY['evaluacion_id','matriz_criterio_id']::TEXT[],NULL::TEXT,NULL::TEXT[]),
      ('cliente_gr_criterio_resultado','fk_cliente_gr_criterio_resultado_evaluacion_matriz','f',ARRAY['evaluacion_id','matriz_version_id']::TEXT[],'cliente_gr_evaluacion',ARRAY['id','matriz_version_id']::TEXT[]),
      ('cliente_gr_criterio_resultado','fk_cliente_gr_criterio_resultado_criterio_matriz_ambito_orden','f',ARRAY['matriz_criterio_id','matriz_version_id','ambito','orden']::TEXT[],'matriz_criterio',ARRAY['id','matriz_version_id','ambito','orden']::TEXT[])
  ...
  LOOP
    SELECT pg_catalog.pg_get_expr(c.conbin,c.conrelid) INTO expression_sql
      FROM pg_catalog.pg_constraint c
     WHERE c.conrelid=pg_catalog.to_regclass('public.' || expected.tabla)
       AND c.conname=expected.nombre AND c.contype='c' AND c.convalidated;
    IF expression_sql IS NULL THEN
  ...
  FOR expected IN
    SELECT * FROM (VALUES
      ('cliente_pt_evaluacion','uq_cliente_pt_evaluacion_id_cliente_empresa_matriz_gr',ARRAY['id','cliente_id','empresa_id','matriz_version_id']::TEXT[]),
  ...
  LOOP
    ...
    IF actual_columns IS DISTINCT FROM expected.columnas THEN
      RAISE EXCEPTION 'VERIFY fallido: auxiliar % incompatible',expected.nombre;
    END IF;
  END LOOP;
  ...
  IF (SELECT pg_catalog.count(*) FROM pg_catalog.pg_constraint c
     ...
     OR (SELECT pg_catalog.count(*) FROM pg_catalog.pg_constraint c
       WHERE c.conrelid='public.cliente_gr_criterio_resultado'::pg_catalog.regclass) <> 9 THEN
    RAISE EXCEPTION 'VERIFY fallido: cantidad de constraints inesperada';
  END IF;

  FOR expected IN
    SELECT * FROM (VALUES
      ('cliente_gr_evaluacion','pk_cliente_gr_evaluacion',ARRAY['id']::TEXT[],true,true),
      ('cliente_gr_evaluacion','uq_cliente_gr_evaluacion_cliente_version',ARRAY['cliente_id','numero_version']::TEXT[],true,false),
      ('cliente_gr_evaluacion','uq_cliente_gr_evaluacion_id_matriz',ARRAY['id','matriz_version_id']::TEXT[],true,false),
      ('cliente_gr_evaluacion','idx_cliente_gr_evaluacion_empresa',ARRAY['empresa_id']::TEXT[],false,false),
      ('cliente_gr_evaluacion','idx_cliente_gr_evaluacion_matriz',ARRAY['matriz_version_id']::TEXT[],false,false),
      ('cliente_gr_evaluacion','idx_cliente_gr_evaluacion_pt',ARRAY['pt_evaluacion_id']::TEXT[],false,false),
      ('cliente_gr_evaluacion','idx_cliente_gr_evaluacion_resultado',ARRAY['matriz_resultado_id']::TEXT[],false,false),
      ('cliente_gr_evaluacion','idx_cliente_gr_evaluacion_creada_por',ARRAY['creada_por']::TEXT[],false,false),
      ('cliente_gr_criterio_resultado','pk_cliente_gr_criterio_resultado',ARRAY['id']::TEXT[],true,true),
      ('cliente_gr_criterio_resultado','uq_cliente_gr_criterio_resultado_evaluacion_criterio',ARRAY['evaluacion_id','matriz_criterio_id']::TEXT[],true,false),
      ('cliente_gr_criterio_resultado','idx_cliente_gr_criterio_resultado_criterio',ARRAY['matriz_criterio_id']::TEXT[],false,false)
  ...
  IF (SELECT pg_catalog.count(*) FROM pg_catalog.pg_index i
    ...
    RAISE EXCEPTION 'VERIFY fallido: cantidad de indices inesperada';
  END IF;
END
$$;
